...
Помощники(далее по тексту хелперы (Helpers)) это классы похожие на Компоненты,
служащие для улучшения слоя отображения. Хелперы 
содержат логику отображения, и могут быть использованы в представлениях(Views), 
Элементах(Elemwnts) и Шаблонах(Layouts). В данной главе показано как
создать свой Хелпер, а так же описаны основные задачи, которые могут быль решены при помощи Хелперов CakePHP.

В CakePHP имеется набор Хелперов, которые помогают в создании представлений.
Они помогают в создании хорошо сформированной разметки (в том числе форм), помогают
при форматировании текста, дат и времени, цифр, и даже могут ускорить функциональность AJAX.
Для получения дополнительной информации о Хелперах, включенных в CakePHP,
посмотрите разделы с документацией для каждого Хелпера:
...
Использование и настройка Хелперов
==================================

Для активации Хелперов в CakePHP необходимо сделать так, чтобы контроллер узнал о них. Каждый контроллер имеет 
свойство :php:attr:`~Controller::$helpers`, которое перечисляет доступные в представлении Хелперы.
Для активации Хелпера в представлении необходимо длбавить его название в его свойство - массив ``$helpers`` array::

    class BakeriesController extends AppController {
        public $helpers = array('Form', 'Html', 'Js', 'Time');
    }

Для добавления Хелперов из плагинов используется :term:`синтаксис плагина`, используемый повсеместно в
CakePHP::
...
Также можно добавлять Хелперы прямо в экшен, чтобы они были доступны только внутри него, и не были доступны
в других экшенах контроллера. Это сохранит вычислительные мощности для других экшенов, 
а также поможет в организации кода ::
